import './App.css';
import DropdownMenu from './components/DropdownMenu/DropdownMenu';
import Element from './components/DropdownMenu/components/Element/Element'
import ICON_PLANNING from './assets/icon-planning.svg'
import ICON_REMINDERS from './assets/icon-reminders.svg'
import ICON_TODO from './assets/icon-todo.svg'

// const ICON_PLANNING = <svg width="16" height="16" xmlns="http://www.w3.org/2000/svg"><path d="M8 0a8 8 0 1 1 0 16A8 8 0 0 1 8 0Zm0 2.133a5.867 5.867 0 1 0 0 11.734A5.867 5.867 0 0 0 8 2.133ZM8 3.2a4.8 4.8 0 1 1 0 9.6 4.8 4.8 0 0 1 0-9.6Zm-.533 2.667a.533.533 0 0 0-.534.533v2.133c0 .295.24.534.534.534h3.2a.533.533 0 0 0 0-1.067H8V6.4a.533.533 0 0 0-.533-.533Z" fill="#8E4CB6"/></svg>
// const ICON_REMINDERS = <svg width="13" height="17" xmlns="http://www.w3.org/2000/svg"><path d="M6.408 13.916c-3.313 0-6-1.343-6-3 0-.612.371-1.18 1-1.654V7.916a5 5 0 0 1 3.041-4.6 2 2 0 0 1 3.507-1.664 2 2 0 0 1 .411 1.664 5.002 5.002 0 0 1 3.041 4.6v1.346c.629.474 1 1.042 1 1.654 0 1.657-2.687 3-6 3Zm0 1c.694 0 1.352-.066 1.984-.16.004.054.016.105.016.16a2 2 0 0 1-4 0c0-.055.012-.106.016-.16.633.094 1.29.16 1.984.16Z" fill="#EDD556"/></svg>
// const ICON_TODO = <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg"><path d="M14 3v12a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1h1V1a1 1 0 1 1 2 0v1h2V1a1 1 0 1 1 2 0v1h2V1a1 1 0 0 1 2 0v1h1a1 1 0 0 1 1 1Zm-2 3H2v1h10V6Zm0 3H2v1h10V9Zm0 3H2v1h10v-1Z" fill="#726CEE"/></svg>

function App() {
  return (
    <div className="App">
      <h1>Dropdown Menu</h1>
      <ul>
        <DropdownMenu title="Features">
          <Element icon={<img src={ICON_PLANNING} width={20} height={20} alt='planning-icon' />}>Planning</Element>
          <Element icon={<img src={ICON_REMINDERS} width={16} height={20} alt='planning-icon' />}>Reminders</Element>
          <Element icon={<img src={ICON_TODO} width={20} height={20} alt='planning-icon' />}>To Do</Element>
          <DropdownMenu title="Others">
            <Element>Item</Element>
            <Element>Item</Element>
            <Element>Item</Element>
          </DropdownMenu>
        </DropdownMenu>
      </ul>

      {/* <DropdownMenu title="First menu">
        <p>🚀 First item</p>
        <p>🎉 Second item</p>
      </DropdownMenu>
      <DropdownMenu title="Second menu">
        <p>item 1</p>
        <p>item 2</p>
        <p>item 3</p>
        <p>item 4</p>
      </DropdownMenu> */}
    </div>
  );
}

export default App;
